...
#if defined(NDEBUG)
# error "climatechangecryptodollars cannot be compiled without assertions."
#endif
...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // climatechangecryptodollars: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "climatechangecryptodollars Signed Message:\n";
...
{
    // climatechangecryptodollars: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 12500 * COIN;

    // Subsidy is cut in half every 500000 blocks, which will occur approximately every 2.25 years
    nSubsidy >>= (nHeight / 500000); // climatechangecryptodollars: 500000 blocks in 2.25 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // climatechangecryptodollars: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // climatechangecryptodollars: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // climatechangecryptodollars: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // climatechangecryptodollars: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "qwe";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 12500 * COIN;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // climatechangecryptodollars: increase each by adding 2 to bitcoin's value.
...
//////////////////////////////////////////////////////////////////////////////
//
// climatechangecryptodollarsMiner
//
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    //// debug print
    printf("climatechangecryptodollarsMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("climatechangecryptodollarsMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("climatechangecryptodollarsMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static climatechangecryptodollarsMiner(CWallet *pwallet)
{
    printf("climatechangecryptodollarsMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("climatechangecryptodollars-miner");
    ...
    try { loop {
        ...
        printf("Running climatechangecryptodollarsMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("climatechangecryptodollarsMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&climatechangecryptodollarsMiner, pwallet));
}
